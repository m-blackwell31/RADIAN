...
class RuleDetector:
    ...
    def _reader_thread(self):
        ...
        # Pass port/config overrides via environment variables
        env = {**os.environ,
               'PYTHONPATH': parser_dir,
               'MMW_CLI_PORT': self.args.cli_port,
               'MMW_DATA_PORT': self.args.data_port,
               'MMW_CFG_FILE': self.args.cfg_file}
        ...
        try:
            ...
            def _fwd():
                for line in proc.stderr:
                    line = line.rstrip()
                    if line: log.info(f"[parser] {line}")
