...
class FallDetector:
    def __init__(self, args):
        ...
        # Allow command-line threshold override
        ...
        # Consecutive fall counter for confirmation gate
        ...
        try:
            while self._running:
                ...
                if (
                    ...
                    # Skip if window looks frozen (all frames identical — startup artifact)
                    with self._lock:
                        npts_vals = [f.get("num_points_filt", 0) for f in self.window]
                    if len(set(npts_vals)) == 1 and npts_vals[0] > 0:
                        if self.args.verbose:
                            log.info("skip  window appears frozen (identical frames)")
                        time.sleep(0.01)
                        continue
                    ...
                    # Only alert after N consecutive fall predictions
                    confirm = getattr(self.args, "confirm", 2)
                    if pred and self._consecutive_falls >= confirm:
                    ...
                    if self.args.verbose or alerted:
                        tag = "ALERT ⚠️" if alerted else "ok    "
                        log.info(
                            f"{tag} prob={prob:.3f}  "
...
def parse_args():
    ...
    p.add_argument("--confirm",      default=2, type=int,
                   help="Consecutive FALL predictions required before alert [default: 2]")
    p.add_argument("--threshold",    default=None, type=float,
                   help="Override model threshold (default: use value from .pkl, recommended: 0.55)")
    return p.parse_args()
