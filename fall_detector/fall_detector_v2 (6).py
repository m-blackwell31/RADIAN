...
# ── background clutter map ────────────────────────────────────────────────────
_BG_VOXELS: set = set()
_BG_VOXEL_SIZE = 0.15   # metres
_BG_SCAN_DONE  = False

def _voxel_key(x, y, z):
    return (
        round(x / _BG_VOXEL_SIZE),
        round(y / _BG_VOXEL_SIZE),
        round(z / _BG_VOXEL_SIZE),
    )

def filter_background(frame: dict) -> dict:
    """Remove points that match the static background map."""
    if not _BG_SCAN_DONE or not _BG_VOXELS:
        return frame
    pts = frame.get("points_filt", []) or []
    kept = [p for p in pts if _voxel_key(p["x"], p["y"], p["z"]) not in _BG_VOXELS]
    if len(kept) == len(pts):
        return frame
    new_frame = dict(frame)
    new_frame["points_filt"]     = kept
    new_frame["num_points_filt"] = len(kept)
    return new_frame
...
class FallDetector:
    def __init__(self, args):
        ...
        if getattr(args, "threshold", None) is not None:
            ...
            log.info(f"Threshold overridden to {self.threshold:.2f}")
        log.info(
    ...
    def run(self):
        ...
        log.info("Starting in 5 seconds — get in front of the radar…")
        time.sleep(5)
        reader = threading.Thread(target=self._reader_thread, daemon=True)
        reader.start()

        # ── Heartbeat thread ──────────────────────────────────────────────
        # Sends a Gotify message every 10s so the mobile app knows the
        # system is alive. Body is JSON: {"type":"heartbeat","timestamp":"..."}
        def _heartbeat_loop():
                ...
                if self.args.gotify_url and self.args.gotify_token:
                    try:
                        ...
                        _req.post(
                            ...
                            json={
                                "title"   : "heartbeat",
                                "message" : _json.dumps({"type": "heartbeat", "timestamp": ts}),
                                "priority": 1,
                            },
                            timeout=5,
                        )
                    except Exception as _e:
                        log.debug(f"Heartbeat failed: {_e}")
        ...
        # ── Background scan ────────────────────────────────────────────────────
        bg_frames = getattr(self.args, "bg_frames", 50)
        if bg_frames > 0:
            log.info(f"Background scan — stand CLEAR of the radar for ~{bg_frames // 10}s…")
            global _BG_VOXELS, _BG_SCAN_DONE
            bg_collected = 0
            while bg_collected < bg_frames and self._running:
                current_bg = self._frame_count
                if current_bg > bg_collected:
                    with self._lock:
                        snap = list(self.window)
                    for frame in snap[bg_collected:]:
                        for p in (frame.get("points_filt") or []):
                            _BG_VOXELS.add(_voxel_key(p["x"], p["y"], p["z"]))
                    bg_collected = current_bg
                time.sleep(0.05)
            _BG_SCAN_DONE = True
            log.info(f"Background scan complete — {len(_BG_VOXELS)} clutter voxels removed")
                            ...
                            try:
                                ...
                                def _ready_beep():
                                    for _ in range(2):
                                        ...
                                        GPIO.output(27, GPIO.LOW)
                                        _time.sleep(0.10)
                                _thr.Thread(target=_ready_beep, daemon=True).start()
                            except Exception as e:
                                log.debug(f"Ready beep error: {e}")
                        # Gotify ready notification
                        if self.args.gotify_url and self.args.gotify_token:
                            try:
                                import requests as _req
                                _req.post(
                                    ...
                                    json={
                                        "title"   : "✅ RADIAN Online",
                                        "message" : "Fall detection system is active and monitoring.",
                                        "priority": 4,
                                    },
                                    timeout=5,
                                )
                            except Exception as e:
                                log.debug(f"Ready notification failed: {e}")
                    else:
                        time.sleep(0.01)
                        continue
...
def parse_args():
    ...
    p.add_argument("--threshold",    default=None, type=float,
                   help="Override model threshold (default: use value from .pkl)")
    p.add_argument("--bg-frames",    default=50, type=int,
                   help="Frames to collect for background scan (0 to disable) [default: 50]")
    return p.parse_args()
